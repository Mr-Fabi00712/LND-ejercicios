...
SELECT nombre AS nombre_de_producto, precio AS euros , precio * 1.08 AS dólares FROM producto;
...
SELECT nombre, UPPER(LEFT(nombre, 2)) AS iniciales_mayusculas FROM fabricante;
...
SELECT distinct id FROM producto;
...
select nombre, precio FROM producto order by nombre ASC LIMIT 1;

-- Lista el nombre y el precio del producto más caro. (Utilice solamente las cláusulas ORDER BY y LIMIT)

select nombre, precio FROM producto order by nombre desc LIMIT 1;
...
SELECT nombre, precio FROM producto where id_fabricante= 1 OR id_fabricante = 3 OR id_fabricante = 5;

-- Lista todos los productos donde el identificador de fabricante sea 1, 3 o 5. Utilizando el operador IN.

SELECT nombre, precio FROM producto where id_fabricante IN (1, 3, 5);
